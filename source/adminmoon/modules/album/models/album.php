...
class AlbumModelsAlbum extends FSModels
{
    ...
    function __construct()
    {
        ...
        $this->limit = 20;
        $this->view = 'album';
        $this->table_category_name = 'fs_album_categories';
        $this->table_types = 'fs_album_types';
        $this->arr_img_paths = array(
            array('tiny', 492, 277, 'resize_image_fix'),
        );
        $this->table_name = 'fs_album';
        $this->img_folder = 'images/album/' . date('Y/m');
        $this->check_alias = 0;
    ...
    function save()
    {
        ...
        $this->_update(array(
            'total_images' => $this->get_count('record_id = "'.intval($id).'"', 'fs_album_images')
        ), 'fs_album','id='.intval($id));

        return $id;
    }

    function updateImages($id=0){
        global $db;
        $session_id = FSInput::get('session_id');
        $query = '  UPDATE fs_album_images SET record_id = '.$id.', tmp = \'\'
