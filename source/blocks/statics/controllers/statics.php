 ...
 */ 
require_once(PATH_BASE.'blocks/statics/models/statics.php');
class StaticsBControllersStatics{
    function display($parameters, $title, $blockId = 0){
        ...
        $bLink = FSRoute::_('index.php?module=statics&view=home&Itemid='.$Itemid);

        $model->category_id = $category_id ? $category_id : 0;
        if ($model->category_id) {
            $cat = $model->getCategory();
            $bLink = FSRoute::_('index.php?module=statics&view=cat&id=' . $cat->id . '&ccode=' . $cat->alias . '&Itemid=' . $Itemid);
        }
        $list = $model->getList();
        //testVar($list);
        require(PATH_BASE.'blocks/statics/views/'.$style.'.php');
    }
}