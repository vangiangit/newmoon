...
class NewsBModelsNews{
    ...
    function getList($category_id = 0){
        ...
        $query = '  SELECT id, title, image, summary, alias, created_time, category_id, category_name, category_alias, category_icon
    ...
    function getCategory(){ 
        ...
        $query = '  SELECT id, name, alias, alias_wrapper, parent_id, list_parents, icon
                    ...
                    WHERE id IN ('.$this->category_id.')'.SQL_LANG;
        $sql = $db->query($query);
		return $db->getObject();
    }
    ...
    function getCategories(){
        ...
        $query = '  SELECT id, name, alias, alias_wrapper, parent_id, list_parents, icon
                    FROM '.$this->table_category.' 
                    WHERE show_in_homepage = 1 '.SQL_LANG;
        $sql = $db->query($query);
